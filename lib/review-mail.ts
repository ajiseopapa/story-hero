...
export interface ReviewMailInput {
  ...
  childName: string;
  honorific: Honorific;
  /** 첫 손님에게만 넣는 문장. 두 번째 손님부터는 반드시 false. */
...
export function reviewRequestMail(input: ReviewMailInput): { subject: string; body: string } {
  ...
  const subject = `${call} 동화책, 잘 보고 계신가요? (키즈북)`;

  const lines = [
    `${parent}께,`,
    ``,
    `안녕하세요. 키즈북을 만든 김태경입니다.`,
    `지난번 ${call} 동화책을 주문해 주셔서 감사했습니다.`,
    ...(input.firstCustomer
      ? [`저희 첫 손님이셔서, 결제 알림이 왔을 때 솔직히 소리를 질렀습니다.`]
      : []),
    ``,
    `저도 부산에서 아이 셋을 키우는 아빠입니다.`,
    `이 서비스를 만든 뒤로 제일 궁금한 게 "다른 집 아이는 자기 책을 받고 어떤 표정이었을까"인데,`,
    input.firstCustomer
      ? `아직 그걸 물어볼 수 있는 분이 ${parent}뿐입니다.`
      : `아직 그걸 물어볼 수 있는 분이 몇 분 되지 않습니다.`,
    ``,
    `그래서 부탁을 하나 드리고 싶습니다. 이 메일에 답장으로 주시면 됩니다.`,
    ``,
    `1. ${call}가 책을 봤을 때 어땠는지, 한두 줄이면 충분합니다.`,
    `2. 가능하시다면 ${call}가 책을 보는 사진이나 책 화면 캡처 한 장.`,
    `3. 이 글과 사진을 키즈북 홈페이지 후기에 올려도 될까요? 번호만 답해 주시면 됩니다.`,
    `   ① 사진·글 모두 괜찮아요 (아이 얼굴은 가려서 올립니다)`,
    `   ② 글만 올려 주세요`,
    `   ③ 올리지 말아 주세요`,
    ``,
    `어떤 답이든 다음 손님한테 이 책을 설명하는 데 큰 도움이 됩니다.`,
    ``,
    `그리고 책 한 권을 더 만들 수 있는 쿠폰을 넣어드립니다.`,
    `${call} 다른 주제로 한 권 더 만드셔도 되고, 형제나 사촌이 있으면 그 아이 책을 만드셔도 됩니다.`,
    `후기를 못 주셔도 쿠폰은 편하게 쓰셔도 됩니다.`,
    ``,
    `쿠폰: ${input.code}`,
    `사용: story.kidstel.co.kr 에서 책을 만들다가 결제 단계에서 입력`,
    `기한: ${until}`,
    ``,
    `읽어주셔서 감사합니다.`,
    `${call}가 책 속 자기 얼굴을 오래 좋아해 주면 좋겠습니다.`,
    ``,
    `${BUSINESS.owner} 드림`,

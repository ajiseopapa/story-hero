...
function ageStoryGuide(age: number): string {
  ...
  if (age <= 6) {
    return [
      ...
      "· 문장: 짧고 리듬감 있게(대략 25자 이내). 한 장면에 정보를 하나만 담아.",
  ...
  if (age <= 9) {
    return [
      ...
      "· 문장: 짧은 문장과 조금 긴 문장을 섞어(대략 40자 이내). 장면당 3문장 안팎.",
  ...
  return [
    ...
    "· 문장: 복문을 써도 좋고 장면당 4문장 안팎. 설명하지 말고 장면으로 보여줘.",
...
// 전 연령 "2~4문장"으로 고정했더니 4세 책도 부모가 읽기 벅찬 길이가 나왔다.
function sceneTextSpec(age: number): string {
  if (age <= 2) return "각 장면 텍스트는 한국어로 1~2문장, 한 문장은 15자 안팎의 아주 짧은 문장으로";
  if (age <= 6) return "각 장면 텍스트는 한국어로 2~3문장, 한 문장은 25자를 넘지 않게";
  if (age <= 9) return "각 장면 텍스트는 한국어로 3문장 안팎, 한 문장은 40자를 넘지 않게";
